...
app.registerExtension({
    name: "dynamic.dynamic_io__fix_v5",
    ...
    async beforeRegisterNodeDef(type__node, data__node, app) {
        ...
        debugLog(`<dynamic_io> [v5] Registering: ${data__node.name}`);
        ...
        type__node.prototype.configure = function (config) {
            debugLog(`<dynamic_io> [v5] configure: ${this.type} (ID: ${this.id})`);
            ...
            if (this._dynamic_io_setup) {
                debugLog(`<dynamic_io> [v5] 工作流加载,执行 syncDynamicPorts`);
        ...
        type__node.prototype.onNodeCreated = function () {
            debugLog(`<dynamic_io> [v5] onNodeCreated: ${this.type} (ID: ${this.id})`);
            ...
            queueMicrotask(() => {
                ...
                if (!this._configured) {
                    debugLog(`<dynamic_io> [v5] 新建节点检测 (queueMicrotask),执行初始端口同步`);
                    syncDynamicPorts(this, meta);
                }
            });
...
function check_meta(meta) {
    ...
    if (DYNAMIC_IO_DEBUG && meta && meta.dynamic_io) {
        debugLog(`<dynamic_io> [v5] check_meta: inputs=${meta.dynamic_io.flag__dynamic_inputs}, outputs=${meta.dynamic_io.flag__dynamic_outputs}`);
    }
    return valid;
}

function setupDynamicIO(node, meta) {
    debugLog(`<dynamic_io> [v5] setupDynamicIO for node ${node.id}`);
    ...
    if (flag__dynamic_inputs) {
        debugLog(`<dynamic_io> [v5] 准备设置输入 widget: ${name__dynamic_inputs_widget}`);
        setupWidget(node, count__fixed_inputs, name__dynamic_inputs_widget, prefix__dynamic_inputs, 'input');
    }
    if (flag__dynamic_outputs) {
        debugLog(`<dynamic_io> [v5] 准备设置输出 widget: ${name__dynamic_outputs_widget}`);
...
function setupWidget(node, count__fixed, widget_name, prefix, type) {
    ...
    if (!widget) {
        debugLog(`<dynamic_io> [v5] 警告: 未找到 widget "${widget_name}"`);
        return;
    }

    if (widget._dynamic_io_hooked) {
        debugLog(`<dynamic_io> [v5] widget "${widget_name}" 已被劫持,跳过`);
        return;
    }

    // 劫持前先保存 Python 定义的原始默认值
    const raw_default_value = widget.value;
    debugLog(`<dynamic_io> [v5] Setup ${type} widget "${widget_name}" | 原始默认值: ${raw_default_value}`);
    ...
    Object.defineProperty(widget, 'value', {
        ...
        set(new_value) {
            ...
            debugLog(`<dynamic_io> [v5] ${type} setter: ${current_target} -> ${num_value} (输入值: ${new_value})`);
            ...
            if (num_value !== current_target) {
                ...
                // 如果因连接锁定,同步实际值
                if (actual !== num_value) {
                    debugLog(`<dynamic_io> [v5] ${type} 因连接锁定,调整至: ${actual}`);
                ...
                debugLog(`<dynamic_io> [v5] ${type} setter: 值未变化,跳过端口更新`);
            }
        },
        configurable: true,
        enumerable: true
    ...
    widget.callback = function (value) {
        debugLog(`<dynamic_io> [v5] ${type} callback 触发: ${value}`);
    ...
    debugLog(`<dynamic_io> [v5] widget "${widget_name}" 劫持完成,current_target: ${widget.value}, 原始默认值已保存: ${widget._dynamic_raw_default}`);
}

// 同步端口: 比较当前实际端口数与 widget 目标值,必要时重建
function syncDynamicPorts(node, meta) {
    debugLog(`<dynamic_io> [v5] Syncing ports for node ${node.id}`);
    ...
    if (flag__dynamic_inputs) {
        ...
        if (widget) {
            ...
            if (widget.value === -1 && widget._dynamic_raw_default !== undefined) {
                ...
                debugLog(`<dynamic_io> [v5] Input 首次初始化,使用原始默认值: ${target_value} (raw: ${widget._dynamic_raw_default})`);
            } else {
                // 工作流加载或后续修改: 使用当前值
                target_value = Math.max(0, Math.min(max__dynamic_ports, parseInt(widget.value) || 0));
                debugLog(`<dynamic_io> [v5] Input 使用当前值: ${target_value}`);
            }
            ...
            debugLog(`<dynamic_io> [v5] Input check: 目标=${target_value}, 当前总计=${current_total}, 固定=${count__fixed_inputs}, 动态=${actual_dynamic}`);

            // 只有在不匹配时才触发 setter
            if (actual_dynamic !== target_value) {
                debugLog(`<dynamic_io> [v5] 输入端口不匹配 (${actual_dynamic} != ${target_value}),触发重建`);
                widget.value = target_value;
            } else {
                debugLog(`<dynamic_io> [v5] 输入端口数量匹配,无需重建`);
            }
        } else {
            debugLog(`<dynamic_io> [v5] 同步时未找到输入 widget: ${name__dynamic_inputs_widget}`);
        }
    }
    ...
    if (flag__dynamic_outputs) {
        ...
        if (widget) {
            ...
            if (widget.value === -1 && widget._dynamic_raw_default !== undefined) {
                target_value = Math.max(0, Math.min(max__dynamic_ports, parseInt(widget._dynamic_raw_default) || 0));
                debugLog(`<dynamic_io> [v5] Output 首次初始化,使用原始默认值: ${target_value} (raw: ${widget._dynamic_raw_default})`);
            } else {
                target_value = Math.max(0, Math.min(max__dynamic_ports, parseInt(widget.value) || 0));
                debugLog(`<dynamic_io> [v5] Output 使用当前值: ${target_value}`);
            }
            ...
            debugLog(`<dynamic_io> [v5] Output check: 目标=${target_value}, 当前总计=${current_total}, 固定=${count__fixed_outputs}, 动态=${actual_dynamic}`);

            if (actual_dynamic !== target_value) {
                debugLog(`<dynamic_io> [v5] 输出端口不匹配 (${actual_dynamic} != ${target_value}),触发重建`);
                widget.value = target_value;
            } else {
                debugLog(`<dynamic_io> [v5] 输出端口数量匹配,无需重建`);
            }
        } else {
            debugLog(`<dynamic_io> [v5] 同步时未找到输出 widget: ${name__dynamic_outputs_widget}`);
        }
    }
}
...
function update_input_ports(node, count__fixed, count__target, prefix, widget_name) {
    try {
        ...
        debugLog(`<dynamic_io> [v5] Update inputs: 固定=${count__fixed}, 当前动态=${current_dynamic}, 目标=${target}, 当前总计=${current_total}`);

        if (target === current_dynamic) {
            debugLog(`<dynamic_io> [v5] 输入端口无需变化`);
            return target;
        }

        // 添加端口 (增量)
        if (target > current_dynamic) {
            debugLog(`<dynamic_io> [v5] 准备添加 ${target - current_dynamic} 个输入端口`);
            for (let i = current_dynamic; i < target; i++) {
                const name = `${prefix}${i}`;
                const exists = node.inputs?.some(inp => inp.name === name);
                if (!exists) {
                    const idx = count__fixed + i;
                    debugLog(`<dynamic_io> [v5] 添加输入端口 [${idx}]: ${name}`);
                    node.addInput(name, "*");
                } else {
                    debugLog(`<dynamic_io> [v5] 输入端口 ${name} 已存在,跳过`);
                }
            }
            node.setDirtyCanvas(true, true);
            return target;
        ...
        if (target < current_dynamic) {
            debugLog(`<dynamic_io> [v5] 准备移除输入端口,从 ${current_dynamic} 减至 ${target} (从后往前检查连接)`);
            ...
            for (let i = current_total - 1; i >= count__fixed + target; i--) {
                ...
                debugLog(`<dynamic_io> [v5] 检查输入端口 [${i}] "${input.name}": 连接状态=${is_connected}, link=${input.link}`);

                if (is_connected) {
                    const locked_count = i - count__fixed + 1;
                    debugLog(`<dynamic_io> [v5] [COUNT 锁] 端口 "${input.name}" 有连接,锁定计数为 ${locked_count},停止移除`);
                    return locked_count;
                }

                debugLog(`<dynamic_io> [v5] 移除输入端口 [${i}]: ${input.name}`);
                node.removeInput(i);
            }

            node.setDirtyCanvas(true, true);
            debugLog(`<dynamic_io> [v5] 成功移除至 ${target} 个动态输入端口`);
            return target;
        }
    } catch (e) {
        errorLog(`<dynamic_io> [v5] Error update_input_ports:`, e);
        return 0;
    }
}
...
function update_output_ports(node, count__fixed, count__target, prefix, widget_name) {
    try {
        ...
        debugLog(`<dynamic_io> [v5] Update outputs: 固定=${count__fixed}, 当前动态=${current_dynamic}, 目标=${target}, 当前总计=${current_total}`);

        if (target === current_dynamic) return target;

        if (target > current_dynamic) {
            debugLog(`<dynamic_io> [v5] 准备添加 ${target - current_dynamic} 个输出端口`);
            for (let i = current_dynamic; i < target; i++) {
                const name = `${prefix}${i}`;
                const exists = node.outputs?.some(out => out.name === name);
                if (!exists) {
                    const idx = count__fixed + i;
                    debugLog(`<dynamic_io> [v5] 添加输出端口 [${idx}]: ${name}`);
                    node.addOutput(name, "*");
                } else {
                    debugLog(`<dynamic_io> [v5] 输出端口 ${name} 已存在,跳过`);
                }
            }
            node.setDirtyCanvas(true, true);
            return target;
        ...
        if (target < current_dynamic) {
            debugLog(`<dynamic_io> [v5] 准备移除输出端口,从 ${current_dynamic} 减至 ${target} (从后往前检查连接)`);
            ...
            for (let i = current_total - 1; i >= count__fixed + target; i--) {
                ...
                debugLog(`<dynamic_io> [v5] 检查输出端口 [${i}] "${output.name}": 连接数=${output.links?.length || 0}`);

                if (has_links) {
                    const locked_count = i - count__fixed + 1;
                    debugLog(`<dynamic_io> [v5] [COUNT 锁] 端口 "${output.name}" 有连接,锁定计数为 ${locked_count},停止移除`);
                    return locked_count;
                }

                debugLog(`<dynamic_io> [v5] 移除输出端口 [${i}]: ${output.name}`);
                node.removeOutput(i);
            }

            node.setDirtyCanvas(true, true);
            debugLog(`<dynamic_io> [v5] 成功移除至 ${target} 个动态输出端口`);
            return target;
        }
    } catch (e) {
        errorLog(`<dynamic_io> [v5] Error update_output_ports:`, e);
        return 0;
    }
}
